...
- Expose the keyword-only `sample_lens_txt2img(...)` hook contract consumed by the canonical txt2img runner while the hook still fails loud.

## Key Files
- `apps/backend/engines/lens/lens.py` — manually-registerable `LensEngine` facade; metadata/bootstrap status and keyword-only `sample_lens_txt2img(...)` not-implemented hook.
...
- Runtime txt2img remains absent; API requests must fail before task creation while this engine is parked. The runner hook exists only as a guarded internal seam for a future unpark tranche.
