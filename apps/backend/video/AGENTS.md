...
- `upscaling/` — SeedVR2 child-process runner (repo-local runtime bootstrap, uniform-padding-aware accelerator admission, full process-group cleanup, and fail-loud output validation).
...
- 2026-08-30: `upscaling/seedvr2.py` admits direct and bounded streaming execution from measured CUDA/MPS memory with upstream uniform-batch padding included, terminates the complete child process group, drains diagnostics to EOF, and validates source-cardinality PNG output without a CPU fallback.
