...
- `upscaling/` — SeedVR2 child-process runner (repo-local runtime bootstrap, exact host/CUDA/MPS phase admission, MPS unified-memory admission, numeric frame identity, full process-group cleanup, and fail-loud output validation).
...
- 2026-08-31: `upscaling/seedvr2.py` admits direct and bounded streaming execution from measured CUDA/MPS memory with concurrent source/target tensors, temporal-padding construction peaks, retained MPS input/latent/output state, and one shared MPS physical-memory budget included; it terminates the complete child process group, drains diagnostics to EOF, and validates contiguous numeric source-cardinality PNG output without a CPU fallback.
