...
## Key Files
- `seedvr2.py` — SeedVR2 child runner (exact upstream geometry/padding and source/target live-set admission, source video -> upstream CLI -> numerically validated PNG output paths).
...
- 2026-08-31: Runner metadata records the selected direct or streaming plan, measured accelerator-memory budget, concurrent source/target per-frame estimates, exact source/target/padded geometry, upstream uniform-padding cardinality, validated numeric output dimensions, and any one smart-fallback selection or direct-child OOM retry. Host admission also includes the complete float32 result, float32 PNG multiplication temporary, and uint8 PNG array. Child diagnostics retain a bounded tail plus cumulative OOM evidence, reach EOF before classification, and keep the inference gate occupied until the process group is physically cleaned.
- Runtime directories:
