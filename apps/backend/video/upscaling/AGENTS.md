...
## Key Files
- `seedvr2.py` — SeedVR2 child runner (exact upstream geometry/padding admission, source video -> upstream CLI -> validated PNG output paths).
...
- 2026-08-30: Runner metadata records the selected direct or streaming plan, measured accelerator-memory budget, exact target/padded geometry, upstream uniform-padding cardinality, validated output dimensions, and any one smart-fallback selection or direct-child OOM retry. Child diagnostics retain a bounded tail plus cumulative OOM evidence, reach EOF before classification, and keep the inference gate occupied until the process group is physically cleaned.
- Runtime directories:
