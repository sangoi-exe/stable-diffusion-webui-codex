...
## Key Files
- `seedvr2.py` — SeedVR2 child runner (source video -> upstream CLI -> validated PNG output paths).
- `__init__.py` — Package marker (no facade exports).

## Notes
- Do not import code from `.refs/**` inside `apps/**`.
- Runner executes the checked-out upstream CLI (`inference_cli.py`) in a task-scoped child process. It must fail loud on checkout/bootstrap errors, missing Python dependencies, model download failures, GPU admission failure, child execution failure, and frame count or size mismatches.
- `streaming=true` selects bounded streaming immediately. `smart_fallback=true` first admits whole-video GPU execution from measured CUDA/MPS availability, then selects one bounded streaming route on capacity rejection or direct-child OOM. If cancellation becomes active after a direct child has already exited OOM, the runner preserves that typed OOM and does not start streaming. The runner has no CPU fallback.
- An immediate task cancellation callback terminates the active child process group. The runner does not materialize source or output video tensors in the backend process.
- 2026-08-27: Runner metadata records the selected direct or streaming plan, measured accelerator-memory budget, validated output dimensions, and any one smart-fallback selection or direct-child OOM retry. Child diagnostics retain a bounded output tail and a cumulative OOM marker observation so later logs cannot erase the one allowed streaming retry. After child exit, the child result is classified before a later cancellation observation. It does not claim in-process runtime telemetry.
- Runtime directories:
