...
Last reviewed on 2026-08-30 during the SeedVR2 R3-A immutable-admission, resource, and atomic-publication tranche.
...
- Public route: `apps/backend/interfaces/api/routers/upscale.py` (`/api/video-upscale`)
  - Before task registration, copies one backend-visible path into a task-owned immutable snapshot, validates exact decoded count/timing, terminal-frame duration, AAC preservation evidence, curated SeedVR2 controls, upstream-matched target geometry, exact-schedule host memory, scratch capacity, and a configured CUDA or MPS device. It does not accept browser-uploaded video bytes.
...
- Runtime and file owners: `apps/backend/video/io/ffmpeg.py`, `apps/backend/video/upscaling/seedvr2.py`, and `apps/backend/video/export/ffmpeg_exporter.py`
  - Respectively own cancellable exact probing/timing/extraction, exact upstream batch/4n+1/overlap resource planning plus complete child-tree cleanup, and relative A/V-origin verification plus one atomic artifact-directory publication under the existing output route.
...
- Video Upscale utility seam: `apps/backend/interfaces/api/routers/upscale.py`, `apps/backend/interfaces/api/tasks/video_upscale_tasks.py`, and `apps/backend/use_cases/video_upscale.py`
  - The router snapshots, validates, and dynamically admits the dedicated source-path request before task creation. The worker owns task lifecycle and final snapshot cleanup. The use case owns one snapshot-bound video-upscale pipeline. Reuse the cancellable video I/O, exact-schedule SeedVR2 runner, atomic verified export, task registry, and SSE owners instead of reactivating vid2vid or adding a browser upload path.
