...
Last reviewed on 2026-08-31 during the SeedVR2 R4-A pinned-suffix, resource-live-set, and filesystem-admission tranche.
...
- Runtime and file owners: `apps/backend/video/io/ffmpeg.py`, `apps/backend/video/upscaling/seedvr2.py`, and `apps/backend/video/export/ffmpeg_exporter.py`
  - Respectively own cancellable exact probing/timing/extraction, exact upstream batch/4n+1/overlap planning across concurrent source/target tensors and float32 PNG conversion plus numeric PNG identity and complete child-tree cleanup, and relative A/V-origin verification plus one atomic artifact-directory publication under the existing output route.
