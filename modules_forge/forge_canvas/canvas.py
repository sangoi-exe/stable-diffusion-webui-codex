...
class ForgeCanvas:
    def __init__(
        ...
        js_code = f"""
async ()=>{{
  const ids=["imageContainer_{self.uuid}","drawingCanvas_{self.uuid}","toolbar_{self.uuid}"];
  ...
  try{{
    new ForgeCanvas("{self.uuid}", {no_upload}, {no_scribbles}, {contrast_scribbles}, {height}, '{scribble_color}', {scribble_color_fixed}, {scribble_width}, {scribble_width_fixed}, {scribble_alpha}, {scribble_alpha_fixed}, {scribble_softness}, {scribble_softness_fixed});
  }}catch(e){{ console.error("ForgeCanvas init error", e); }}
}}
"""
        Context.root_block.load(None, js=js_code)
